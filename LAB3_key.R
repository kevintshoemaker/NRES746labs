...
quantilefunc <- function(x,pars,q){    # helper function for plotting quantiles using curve function (not needed here)
  ifelse(x>0.5,
...
Rffuncresp <- function(params=inits,data=rffr){
  temp <- suppressWarnings( optim(fn=binomNLL2,  par=params, N=data[,1], k=data[,2])   )
  MLE <- temp$par
  plot(data[,2]~data[,1],ylab="Killed",xlab="Init Density")
  lines(data[,1],qbinom(0.975,prob=Holl2(data[,1],MLE["a"],MLE["h"])/data[,1],size=data[,1]),col="green",lty=2)
  lines(data[,1],qbinom(0.025,prob=Holl2(data[,1],MLE["a"],MLE["h"])/data[,1],size=data[,1]),col="green",lty=2)
