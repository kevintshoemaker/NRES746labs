...
Please submit the R script and the Word document via WebCampus by midnight on the due date (one week after the final lab session allocated for this topic -- here, _Oct. 8_). You can work in groups but please submit the materials individually. 
...
If you haven't already, please list your final project information (project title, group members etc.) on our [shared Google sheets document](https://docs.google.com/spreadsheets/d/1A0zod4_gKKqNBHOtgNhLcdZlz2JV84pjBv1ajtGcYDc/edit?usp=sharing)
...
+ **input**:         
    ...
    + stdev = a scalar specifying the standard deviation of the residual errors (which we assume are normally distributed).
...
+ **suggested algorithm**:   
    ...
    + Use the linear model defined by the "coefs" variable (response = intercept + slope*covariate) to determine the expected value of the response variable at each covariate value. 
...
```{r q2.1a-solution, echo=FALSE, context='data'}
...
grade_this({
  ...
  fail_if(!names(.result(sample.size=5,xlimits=c(0,12),coefs=c(-4,0.11),stdev=1)==2)%in%c("x","y"), "your function output should produce a data frame with columns labeled x and y")
...
+ **input**:         
    + sim.dataset = a data frame representing data simulated from a linear model with known coefficients (and known residual variance).  The first column, labeled 'x', should represent the values of the "x" variable and the second column, labeled 'y', should represent stochastic observations from the *known* model defined by "trueparams". NOTE: the structure of this input argument should match the output produced by your 'LMDataSim' function.           
    + trueparams = a vector of length 3 specifying the intercept, slope, and standard deviation of the residual errors of the known relationship between "x" and "y"     
...
+ **return**:    
    + out = a list of length 3 with the following elements (with the names indicated below):   
        ...
        + fittedparams = a vector of length 3 representing the fitted parameters (intercept, slope, sd of residual error) from the linear regression model   
...
```{r test2.1b, eval=FALSE,context="data"}
...
```{r test2.1b2, eval=FALSE, context="data"}

simdat2 <- LMDataSim(sample.size=100,xlimits=c(0,15),coefs=c(40,-2.2),stdev=1.8)

test <- LMVerify_pt1(sim.dataset = simdat2, trueparams = c(40,-2.2,1.8), plot=T)

```


```{r q2.1b-check,echo=F}
...
grade_this({
  ...
  fail_if(!names(.result(sim.dataset = simdat, trueparams = c(55,-0.09,6.8), plot=T))%in%c("trueparams","fittedparams","inside"), "your function output should be a list with three elements with proper names (see instructions)")
...
+ **input**:         
    ...
    + sd_coefs = a vector of length 2 specifying (in order) coefficients for a "heteroskedasticity" model specifies the standard deviation of the residuals as a linear function of the covariate "x" (in this data generating model, the standard deviation of the residuals changes linearly with the covariate "x")
...
+ **input**:         
    + sim.dataset = a data frame representing data simulated from a linear model with known coefficients (and known residual variance).  The first column, labeled 'x', should represent the values of the "x" variable and the second column, labeled 'y', should represent stochastic observations from the *known* model defined by "trueparams". NOTE: the structure of this input argument should match the output produced by your 'LMDataSim' function.           
    + trueparams = a vector of length 2 specifying the intercept and slope of the known relationship between "x" and "y"  [note: this time we don't include the true residual error, which varies with 'x']          
    + plot = a logical (TRUE/FALSE) indicating whether or not to produce a plot with regression line and confidence interval 
...
grade_this({
  ...
  fail_if(!names(.result(sim.dataset = simdat3, trueparams = c(55,-0.19),plot=T))%in%c("trueparams","fittedparams","inside"), "your function output should be a list with three elements with proper names (see instructions)")
...
2. Evaluate *goodness-of-fit* by visualizing the range of data produced by the true model (using a very large sample size) against the prediction interval from the estimated model (generated using the 'lm()' function). See the code and figure below for reference. In your Word document, please describe briefly how this figure helps you to evaluate goodness-of-fit (i.e., could your observed data have been generated by the model? Are there 'red flags' that might make you think that the model is not adequate for representing these data?). 
...
```{r answer2.3a0, echo=FALSE, context='data'}
...
GetPower <- function(initabund=1000,nreps=1000,trend=-0.03,years=25,days=3,observers=1,survint=2,alpha=0.05,verbose=T){
