...
Please submit the R script and the Word document via WebCampus by midnight on the due date (_Oct. 5_). You can work in groups but please submit the materials individually. 
...
grade_this({
  ...
  fail_if(!names(.result(sample.size=5,xlimits=c(0,12),coefs=c(-4,0.11),stdev=1))%in%c("x","y"), "your function output should produce a data frame with columns labeled x and y")
...
+ **input**:         
    ...
    + sd_coefs = a vector of length 2 specifying (in order) coefficients for a model that specifies the standard deviation of the residuals (magnitude of residual error) as a linear function of the covariate "x" (in this data generating model, the standard deviation of the residuals changes linearly with the covariate "x")
...
grade_this({
  ...
  fail_if(!names(.result(sample.size=5,xlimits=c(0,12),coefs=c(-4,0.11),sd_coefs=c(0.2,0.04)))%in%c("x","y"), "your function output should produce a data frame with columns labeled x and y")
...
```{r answerq2_2c1, echo=FALSE, eval=FALSE}
