...
First, take a little time to review the [likelihood lecture](http://naes.unr.edu/shoemaker/teaching/NRES-746/LECTURE4.html)!

## Example: reed frog predation data

First, load the reed frog predation data from the Bolker book- it can be found [here](http://naes.unr.edu/shoemaker/teaching/NRES-746/ReedfrogPred.csv). Save this file to your working directory.
...
```{r q3.1a-check, echo=F}
check_q3.1a <- function(USER_CODE){
...
Save the reed frog functional response dataset to your working directory- it can be found [here](http://naes.unr.edu/shoemaker/teaching/NRES-746/ReedfrogFuncresp.csv). 
...
```{r q3.2a-check, echo=F}
check_q3.2a <- function(USER_CODE){
    code <- checkr::for_checkr(USER_CODE)
...
2. Bolker calls the prediction interval you generated above a *plug-in prediction interval*. In what way(s) is this interval different than a *true* prediction interval? (hint: are there are any sources of error that the plug in interval ignores?)   
...
```{r q3.3a-check, echo=F}
check_q3.3a <- function(USER_CODE){
    code <- checkr::for_checkr(USER_CODE)
...
```{r q3.3b, exercise=TRUE}
MyxRicker <- function(params,data){
    # [add code here!]
}
MyxRicker(MyxRicker(params=c(a=1,b=0.1,shape=30),data=myxdat[,-1]))

```


```{r q3.3b-check, echo=F}
check_q3.3b <- function(USER_CODE){
    code <- checkr::for_checkr(USER_CODE)
...
```{r q3.4b-check, echo=F}
check_q3.4b <- function(USER_CODE){
    code <- checkr::for_checkr(USER_CODE)
