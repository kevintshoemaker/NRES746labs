...
CLTdemo <- function(n.samples=1000,sample.size=10,min=10,max=20){
  ...
  for(i in 1:n.samples){      # for each replicate (independent random sample)
    sample <- runif(sample.size,min,max)   # draw an independent random sample from the population of interest
    samplemean[i] <- mean(sample)    # record the sample mean
  }
  
  
  par(mfrow=c(1,2))
  hist(runif(1000,min,max),freq=F,ylim=c(0,1),main="",xlab="Value")      # plot out the distribution of underlying data
  hist(samplemean,freq=F,add=T,col="red")    # overlay the distribution of the sample means (sampling distribution).
...
```{r q1d-check}
grade_this({
  fail_if(!.result$method=="Shapiro-Wilk normality test","Make sure your function returns the results from the Shapiro-Wilk normality test!")
  pass_if(near(.result$statistic,.solution$statistic,0.0006))
  fail()
})
```
...
my.sample = c(5.14,3.27,4.56,3.77,3.34,4.32,3.84,3.19,5.24,4.09)
grade_this({
  fail_if(!names(.result)%in%c("null_dist","p_value","observed_mean"),"check the list names for your returned object")
...
```{r 5a-solution, echo=FALSE, context='data'}
