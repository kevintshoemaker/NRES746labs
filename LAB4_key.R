...
# NOTE: need to revamp this lab for automated grading- need more encapsulation. careful about 'densityplot'
...
Myx_Ricker_JAGS <- function(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.95){
  ...
  jags.fit_1a <- jags(data=myx.data.for.bugs,inits=init.vals.for.bugs,parameters.to.save=params.to.store,n.iter=50000,model.file=JAGScode,
                      n.chains = 3,n.burnin = 5000,n.thin = 20,parallel=TRUE )
  jags.fit_1a_coda <- jags.fit_1a$samples
  #summary(jagsfit.mcmc.1a)
  plot(jags.fit_1a_coda)
  #temp <- densityplot(jags.fit_1a)
  print(temp)
...
Myx_MM_JAGS <- function(data=Myx[,-1],JAGScode="BUGSmodel_mm.txt",CImethod="HPD",CIlevel=0.95){
  ...
  jags.fit_2a <- jags(data=myx.data.for.bugs,inits=init.vals.for.bugs2,parameters.to.save=params.to.store,n.iter=50000,
                      model.file=JAGScode,n.chains = 3,n.burnin = 5000,n.thin = 20,parallel=TRUE )
...
Myx_PostPredCheck <- function(MCMC1=jags.fit_ricker,MCMC2=jags.fit_mm){
  lots <- 1000
  nMCMC <- min(length(MCMC1$sims.list[[1]]),length(MCMC2$sims.list[[1]]))
