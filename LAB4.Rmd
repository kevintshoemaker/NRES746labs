...
```{r q4.1a-solution, echo=FALSE, context="data"}
...
```{r dataq4.1a0, echo=FALSE, context="data"}
...
grade_this({
  ...
  fail_if(!near(.result(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[1]][1],
              .solution(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[1]][1],0.1))
  pass_if(near(.result(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[2]][1,1],
              .solution(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[2]][1,1],0.1))
  fail()
})

# check_q4.1a <- function(USER_CODE){
...
# }
# check_q4.1a(USER_CODE)
```

```{r playwithcheckr4, eval=FALSE, echo=FALSE}

code <- checkr::for_checkr(quote({
...
this <- Myx_MM_JAGS(data=Myx,JAGScode = "BUGSmodel_mm.txt", CImethod = "HPD", CIlevel = 0.95)
test<-this$conf.ints
}))
code
...
```{r q4.2a-check, echo=F}
check_q4.2a <- function(USER_CODE){
    code <- checkr::for_checkr(USER_CODE)
    func_call <- checkr::line_where(code,checkr::insist(F== "Myx_MM_JAGS","Use Myx_MM_JAGS() function!"))
    line_call <- checkr::line_where(code,checkr::insist(Z=="test","refresh the sandbox and please don't mess with the test code!"))
    t1 <- checkr::line_where(line_call, 
...
+ **suggested algorithm**:  
    + For each model (Ricker, M-M), generate new data under the fitted model. For each of an arbitrarily large number (e.g., 1000) of replicates, sample from the joint posterior distribution:     
        ...
        + Then, use that random index to select a single random sample from the joint posterior distribution. This will look something like this: `new.draw <- jags.fit$sims.list$a[random_index]`    
...
```{r answer4.3a1, echo=FALSE, context="data"}
...
```{r q4.3a-check, echo=F}
check_q4.3a <- function(USER_CODE){
    code <- checkr::for_checkr(USER_CODE)
