...
First, take a little time to review the [Bayesian inference lecture](http://naes.unr.edu/shoemaker/teaching/NRES-746/LECTURE6.html) and [MCMC lecture](http://naes.unr.edu/shoemaker/teaching/NRES-746/LECTURE7.html)!   

**Lab 4 is due before midnight on Tuesday November 16** 

##  Bayesian inference (with BUGS!)

If you haven't already installed JAGS on your computer, use [this link](http://mcmc-jags.sourceforge.net/). You need to install JAGS on your computer -- loading R packages like 'jagsUI' is not enough.            

In this lab we will be applying a Bayesian approach to model fitting using the same myxomatosis dataset and data generating model that we used in the previous lab (lab 3: maximum likelihood inference). To do this, we’ll be using the program JAGS (**J**ust **A**nother **G**ibbs **S**ampler).
...
```{r dataq4_1a0, echo=FALSE, context="data"}
...
Myx_Ricker_JAGS <- function(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.95){
  ...
  jags.fit_1a <- jags(data=myx.data.for.bugs,inits=init.vals.for.bugs,parameters.to.save=params.to.store,n.iter=50000,model.file=JAGScode,n.chains = 3,n.burnin = 5000,n.thin = 20,parallel=TRUE )
  jags.fit_1a_coda <- jags.fit_1a$samples
  #summary(jagsfit.mcmc.1a)
  plot(jags.fit_1a_coda)
  #temp <- densityplot(jags.fit_1a)
...
```{r q4_1a-check, echo=F}

grade_this({
  fail_if(!length(.result(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88))==3, "your function output should be a list with three elements")
  fail_if(!near(.result(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[1]][1],
              .solution(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[1]][1],0.2))
  pass_if(near(.result(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[2]][1,1],
              .solution(data=Myx[,-1],JAGScode="BUGSmodel_ricker.txt",CImethod="HPD",CIlevel=0.88)[[2]][1,1],0.2))
  fail()
})
...
Myx_MM_JAGS <- function(data=Myx[,-1],JAGScode="BUGSmodel_mm.txt",CImethod="HPD",CIlevel=0.95){
  ...
  jags.fit_2a <- jags(data=myx.data.for.bugs,inits=init.vals.for.bugs2,parameters.to.save=params.to.store,n.iter=50000,model.file=JAGScode,n.chains = 3,n.burnin = 5000,n.thin = 20,parallel=TRUE )
  jags.fit_2a_coda <- jags.fit_2a$samples
  plot(jags.fit_2a_coda)
  temp <- densityplot(jags.fit_2a_coda)
  print(temp)
...
grade_this({
  fail_if(!length(.result(data=Myx[,-1],JAGScode="BUGSmodel_mm.txt",CImethod="HPD",CIlevel=0.88))==3, "your function output should be a list with three elements")
  fail_if(!near(.result(data=Myx[,-1],JAGScode="BUGSmodel_mm.txt",CImethod="HPD",CIlevel=0.88)[[1]][1],
              .solution(data=Myx[,-1],JAGScode="BUGSmodel_mm.txt",CImethod="HPD",CIlevel=0.88)[[1]][1],0.5))
  pass_if(near(.result(data=Myx[,-1],JAGScode="BUGSmodel_mm.txt",CImethod="HPD",CIlevel=0.88)[[2]][1,1],
              .solution(data=Myx[,-1],JAGScode="BUGSmodel_mm.txt",CImethod="HPD",CIlevel=0.88)[[2]][1,1],0.5))
  fail()
})
```
